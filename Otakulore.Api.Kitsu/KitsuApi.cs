...
namespace Otakulore.Api.Kitsu
{
    ...
    public static class KitsuApi
    {
        ...
        public static async Task<KitsuResponse<KitsuAnimeAttributes>> FilterAnime(string query)
        {
            var response = await RestClient.GetAsync(BaseEndpoint + "/anime?filter[text]=" + Uri.EscapeDataString(query));
            response.EnsureSuccessStatusCode();
            var content = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<KitsuResponse<KitsuAnimeAttributes>>(content)!;
        }
